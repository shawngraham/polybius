                      ...
                      <div className="p-6 pt-2 border-t border-zinc-100 space-y-6 animate-in slide-in-from-top-1 duration-200">
                        ...
                        {section.cardType !== CardType.TEXT && (<div className="bg-zinc-50 rounded-xl p-5 border border-zinc-100">
                           ...
                           <div className="grid grid-cols-2 gap-y-4 gap-x-6">
                              ...
                              {section.cardType === CardType.TIMELINE && (<><div><label className="block text-[10px] font-semibold text-zinc-500 mb-1">Date Column</label><select value={section.config.dateKey || ''} onChange={(e) => updateSectionConfig(section.id, 'dateKey', e.target.value)} className="w-full px-2 py-1.5 border border-zinc-200 rounded bg-white text-xs">{dataHeaders.map(h => <option key={h} value={h}>{h}</option>)}</select></div><div><label className="block text-[10px] font-semibold text-zinc-500 mb-1">Orientation</label><select value={section.config.orientation || 'vertical'} onChange={(e) => updateSectionConfig(section.id, 'orientation', e.target.value)} className="w-full px-2 py-1.5 border border-zinc-200 rounded bg-white text-xs"><option value="horizontal">Horizontal</option><option value="vertical">Vertical</option></select></div></>)}
                              
                              {section.cardType === CardType.MAP && (<><div className="col-span-1"><label className="block text-[10px] font-semibold text-zinc-500 mb-1">Latitude</label><select value={section.config.latKey || ''} onChange={(e) => updateSectionConfig(section.id, 'latKey', e.target.value)} className="w-full px-2 py-1.5 border border-zinc-200 rounded bg-white text-xs">{dataHeaders.map(h => <option key={h} value={h}>{h}</option>)}</select></div><div className="col-span-1"><label className="block text-[10px] font-semibold text-zinc-500 mb-1">Longitude</label><select value={section.config.lngKey || ''} onChange={(e) => updateSectionConfig(section.id, 'lngKey', e.target.value)} className="w-full px-2 py-1.5 border border-zinc-200 rounded bg-white text-xs">{dataHeaders.map(h => <option key={h} value={h}>{h}</option>)}</select></div><div><label className="block text-[10px] font-semibold text-zinc-500 mb-1">Base Map</label><select value={section.config.baseMap || 'terrain'} onChange={(e) => updateSectionConfig(section.id, 'baseMap', e.target.value)} className="w-full px-2 py-1.5 border border-zinc-200 rounded bg-white text-xs"><option value="terrain">Terrain</option><option value="toner">Toner</option><option value="satellite">Satellite</option><option value="voyager">Voyager</option><option value="watercolour">Watercolour</option></select></div><div className="col-span-2 border-t border-zinc-200 pt-3 mt-1"><label className="block text-[10px] font-semibold text-zinc-500 mb-2">Default View (optional - leave blank to auto-fit data)</label><div className="grid grid-cols-3 gap-3"><div><label className="block text-[10px] text-zinc-400 mb-0.5">Centre Lat</label><input type="number" step="any" value={section.config.defaultLat ?? ''} onChange={(e) => updateSectionConfig(section.id, 'defaultLat', e.target.value)} placeholder="e.g. 45.0" className="w-full px-2 py-1.5 border border-zinc-200 rounded bg-white text-xs" /></div><div><label className="block text-[10px] text-zinc-400 mb-0.5">Centre Lng</label><input type="number" step="any" value={section.config.defaultLng ?? ''} onChange={(e) => updateSectionConfig(section.id, 'defaultLng', e.target.value)} placeholder="e.g. -75.0" className="w-full px-2 py-1.5 border border-zinc-200 rounded bg-white text-xs" /></div><div><label className="block text-[10px] text-zinc-400 mb-0.5">Zoom (1-18)</label><input type="number" min={1} max={18} value={section.config.defaultZoom ?? ''} onChange={(e) => updateSectionConfig(section.id, 'defaultZoom', e.target.value)} placeholder="e.g. 5" className="w-full px-2 py-1.5 border border-zinc-200 rounded bg-white text-xs" /></div></div></div></>)}
                              
                              {section.cardType === CardType.NETWORK && (
                                <>
                                  <div>
                                    <label className="block text-[10px] font-semibold text-zinc-500 mb-1">Connections Column</label>
                                    <select value={section.config.connectionsKey || ''} onChange={(e) => updateSectionConfig(section.id, 'connectionsKey', e.target.value)} className="w-full px-2 py-1.5 border border-zinc-200 rounded bg-white text-xs">
                                      {dataHeaders.map(h => <option key={h} value={h}>{h}</option>)}
                                    </select>
                                  </div>
                                  <div>
                                    <label className="block text-[10px] font-semibold text-zinc-500 mb-1">Min Edge Weight Filter</label>
                                    <input 
                                      type="number" 
                                      min={1} 
                                      value={section.config.minWeight || 1} 
                                      onChange={(e) => updateSectionConfig(section.id, 'minWeight', parseInt(e.target.value) || 1)} 
                                      className="w-full px-2 py-1.5 border border-zinc-200 rounded bg-white text-xs" 
                                    />
                                    <p className="text-[9px] text-zinc-400 mt-0.5">Filter connections with weight below this value.</p>
                                  </div>
                                </>
                              )}
                  ...
                  <div className="bg-white p-8 rounded-2xl border border-zinc-200 shadow-sm">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="border-l-4 border-indigo-500 pl-4 py-1">
                        ...
                        <p className="text-sm text-zinc-700 mb-2">Displays one image from the dataset. Uses the same **Image URL** column as the Gallery View. Enter the **Item ID** — the value from your data's ID column — to select which record's image to show. The match is case-insensitive (e.g., "m30" and "M30" both work).</p>
                        <p className="text-xs text-zinc-500 leading-relaxed">Use this when a section's narrative refers to a specific artifact, location, or document and you want to present it without gallery navigation. If the ID is left blank or not found, the first available image is shown.</p>
                      </div>
